...
def train_model(model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        #using ring reduce based all reduce operation and getting the gradients.
        allReduce(model)
...
#function to do the all reduce operations
def allReduce(model):
    for param in model.parameters():
        #getting the grads for each layer.
        grads = torch.as_tensor(param.grad)
        #call all reduce function to calculate sum of gradients from all workers
